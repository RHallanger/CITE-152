...
def ssh_connect(target_ip, password):
    try:
        ...
        # Check for passwordless devices
        for device_name, device_info in devices.items():
            if device_info["ip"] == target_ip:
                # If the device has no password (password is None), accept the connection immediately
                if device_info["password"] is None:
                ...
                # Otherwise, check the provided password
...
def main():
    ...
    while True:
        ...
        # Handle SSH command
            ...
            target_ip = command.split(" ")[-1]
            # Check if the device is passwordless or requires a password
