...
def display_prompt(device_name, ip, device_type):
    ...
    if device_type == "desktop":
        ...
        print("2. 'ipconfig /all' - Display the IP configuration of the desktop.")
...
def execute_command(device, command, current_device):
    ...
    elif command == "ipconfig /all" and device["type"] == "desktop":
        print(f"IP Configuration for {current_device}:")
        print(f"IP Address: {device['ip']}")
        # Safely access the gateway information
        gateway_device = devices.get(device.get("gateway"))
        if gateway_device:
...
def ssh_connect(target_device_ip, password):
    for device_name, device_info in devices.items():
        if device_info["ip"] == target_device_ip:
            ...
            if required_password and password == device_name[1]:  # Password derived from hint
