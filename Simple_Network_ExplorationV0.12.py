...
def main():
    ...
    while True:
        ...
        # Handle RDP command (not implemented here but add logic if required)
        if command.startswith("rdp "):
            target_ip = command.split(" ")[1]
            for device_name, device in devices.items():
                if device["ip"] == target_ip and device["type"] == "desktop":
                    print(f"Connecting to {device_name} via RDP...")
                    break
            else:
                print("No valid desktop found with that IP.")
