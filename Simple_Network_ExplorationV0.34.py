...
def ssh_connect(device_name, target_ip):
    device = devices.get(device_name)
    if not device:
        print(f"Device {device_name} not found.")
        return

    # Get the correct password for the device (routers have no password)
    password = device.get('password')
    
    # Check if the device is a router and if it doesn't have a password
    if device["type"] == "router" and not password:
        print(f"Connecting to {device_name} ({target_ip}) via SSH without a password.")
        return  # Automatically connect since no password is required for routers
    
    # If the device has a password, prompt for it
    if password:
        print(f"Attempting SSH connection to {target_ip}...")
        # Prompt for the password (simulating SSH login)
        entered_password = input(f"Enter password for {device_name}: ")

        if entered_password == password:
            print(f"Connected to {device_name} ({target_ip}) via SSH.")
        else:
            print("Incorrect password. Connection failed.")
    else:
        # If there's no password (for routers) and the player presses Enter, we connect
        print(f"Connecting to {device_name} ({target_ip}) via SSH without a password.")
