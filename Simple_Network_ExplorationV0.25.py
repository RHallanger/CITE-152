        ...
        if command == "show run":
            if device["type"] == "switch":
                ...
                for i in range(0, 3):  # Limiting to three interfaces
                    ...
                    print(f"Description: Password hint for adjacent device: {password_hints.get(device['connected_devices'][i % len(device['connected_devices'])], 'No hint available')}")
            return True

        elif command == "show lldp neighbors":
            print("Listing connected devices:")
            for connected_device in device["connected_devices"]:
                if connected_device != "Router 1" and connected_device != "Router 2":
                    print(f"{connected_device} connected on eth0")
            return True
