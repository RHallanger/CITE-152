    ...
    try:
        if command == "show run":
            if device["type"] == "switch":
                ...
                print("Router interfaces listed. Showing configurations:")
                # Simulating router interfaces with random FastEthernet or GigabitEthernet and limited identifiers
                for i in range(0, 3):  # Limiting to three interfaces
                    ...
                    # Removed the "Password hint for adjacent device:" text
            ...
            for connected_device in device["connected_devices"]:
                # Print the interface to which the device is connected, assuming 'interface' in the device info
                interface = random.choice(["eth0", "GigabitEthernet0/1", "FastEthernet1/2"])
                print(f"{connected_device} connected on {interface}")
            return True
            ...
            if device["type"] == "router":
                ...
                for i in range(1, 11):
                    print(f"{random.choice(['Switch', 'Router', 'Desktop'])} 192.168.1.{i}")
            else:
