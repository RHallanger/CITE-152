    ...
    try:
        if command == "show run":
            ...
            if device["type"] == "switch" or device["type"] == "router":
                ...
                # Generate interface information with specific adjustments for Router 2
                if current_device == "Router 2":
                    # Assign unique interfaces for Switch 4 and Switch 5, with a password hint for Switch 5
                    interfaces = [
            ...
            if "interfaces" not in device:
                device['interfaces'] = []  # Initialize interfaces if not present
                ...
                if i < len(device['interfaces']):
                    ...
                    while True:
                        ...
                        if (interface_type, interface_id) not in assigned_interfaces:
                            ...
                            device['interfaces'].append((interface_type, interface_id, None))  # Add dynamically created interface
                            break
