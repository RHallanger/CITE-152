...
devices = {
    ...
    "Switch 4": {
        ...
        "gateway": "Router 2",  # Now connected to Router 2
        "password": "SSH",
    ...
    "Router 1": {
        "type": "router",
        "ip": "192.168.1.1",
        "connected_devices": ["Switch 1", "Switch 2", "Switch 3", "Switch 5", "Router 2"],  # Switch 5 now on a unique interface
        "gateway": None
    },
    "Router 2": {
        "type": "router",
        "ip": "192.168.2.1",
        "connected_devices": ["Switch 4"],  # Switch 4 now connected to Router 2
        "gateway": "Router 1"
    }
}
...
def execute_command(device, command, current_device, visited_devices):
    ...
    try:
        if command == "show run":
            ...
            # Track unique interface assignment for each connected device
            if "interfaces" in device:
                ...
                for i, connected_device in enumerate(device["connected_devices"]):
                    # Reuse stored interfaces for devices already defined in 'show run'
                    if i < len(device['interfaces']):
                        interface_type, interface_id = device['interfaces'][i]
                    else:
                        # Assign unique interface if not defined in 'show run'
                        while True:
                            interface_type = random.choice(["FastEthernet", "GigabitEthernet"])
                            interface_id = f"{random.randint(0, 2)}/0/{random.randint(0, 24)}"
            ...
            return False  # Handle reconnection to Desktop 1 in the main loop
